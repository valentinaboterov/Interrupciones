...
// Array con los números de los pines donde están conectados los LEDs
int leds[5] = {8, 9, 10, 11, 12};
 
void setup() {
  // Se inicializan los pines de los LEDs como salida y a estado bajo
  for (int i = 0; i < 5; i++)
  {
  ...
  // Configuramos los pines de interrupciones para que detecten un cambio de bajo a alto(RISING)
...
// Recorremos el array de derecha a izquierda
  for (int i = 4; i >= 0; i--)
  {
    // En el primer caso como ya está apagado el LED no hacemos nada(Menor que 4 para no salir del rango(Máximo 3))
    if (i < 4)
    {
